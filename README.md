...
- **하드 검사 8종**(결정론·오차 0): ①주문번호 형식 ②결제 자산 ③종목(허용 목록 **및**
  지금 주문한 그 종목) ④수취인 allowlist ⑤**단위**(청구서가 말하는 `decimals` — 금액은
  base units 정수로 대조하는데 그 정수의 단위를 상대가 채운다) ⑥금액(합의 견적과 base
  units 정합, 오차 0) ⑦**의도 지출 상한**(브로커 견적과 독립적인, 구매 에이전트 자신이
  정한 상한) ⑧건별 한도
  — 세는 기준: `check_demand()` 안에서 서로 다른 이유로 서명을 거부시킬 수 있는 검사 1개를
  1종으로 센다(`payments/guard.py`). 매도 레그의 `check_stock_transfer()` 와 정산 후
  `check_delivery()` 는 이 8종에 포함하지 않고 아래에 따로 적는다.
...
- **실데이터 시세 재생(ReplayPriceFeed)**: 실제 미국 주식 일봉 CSV를 1틱=1봉으로 재생
  ...
  **지표 기준** — 매수: MA5 대비 −3% / 매도: 평단 대비 +5% 익절 (웹 세션 기본값 ·
  화면에서 조정 가능. 아래 `run_demo.py` CLI 는 −2% / +3% 입니다)
...
3. **막는 것은 '정직한 구매 에이전트가 악성 청구서에 속는 것'입니다.** 악성·버그 있는 엔진이
   ...
   그 이관의 첫 조각(예산 상한의 체인 집행)은 `scripts/demo_delegation.py` 로 **레일만
   검증한 상태**이고, 엔진 결제 경로는 아직 오프체인입니다.
...
├── payments/
│   ├── guard.py            # ★ 402 Guard — 서명 직전 게이트 (하드 검사 8종 + 배송 검증)
...
⚠ **저장소에 함께 담긴 시세 데이터(`data/market/*.csv`)는 MIT 적용 대상이 아닙니다.**
오프라인에서 같은 가격 계열을 그대로 재생해 데모·백테스트·심사를 재현할 수 있게 넣어 둔
Alpha Vantage 일봉이며, 재배포용이 아니라 이 프로젝트의 시연·평가용입니다. 다른 용도로
필요하면 출처에서 직접 받으세요(무료 키로 충분합니다 - `scripts/fetch_market_data.py`).
자세한 조건과 "실자금이 관여하지 않는다"는 고지는 [`NOTICE`](NOTICE) 에 있습니다.
