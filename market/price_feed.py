...
class IntradayReplayFeed(ReplayPriceFeed):
    ...
    실제 일봉(시가·고가·저가·종가)을 웨이포인트로 삼은 결정론적 경로를 만든다:
    ...
    ⚠ 정확히 말하면(BUG-10): 연속 경로 자체는 네 웨이포인트를 지나지만, 아래
    `_explode` 는 그 경로를 t=(k+1)/sub 의 **이산 격자에서만 표본화**하고 각 인트라바의
    high/low 를 두 끝점의 max/min 으로만 잡는다. 고가·저가 웨이포인트는 t=1/3·2/3 에
    있으므로 **sub 가 3의 배수일 때만 격자에 걸린다.** UI 선택지는 2/4/8 이라
    그 경우 하루치 인트라바를 집계한 고가·저가가 실제 일봉의 고가·저가와 다르다
    (실측 예: 실 O/H/L/C 100/110/95/108 → sub=2 집계 108.00/100.00, sub=4·8 은
    109.50/96.25, sub=3·6·9 만 110.00/95.00 으로 일치). 종가 경로는 전 구간 정확하다.
    이 값은 캔들차트와 (ta_mode=True 일 때) 지지/저항 피벗 입력에 쓰이고,
    판단·정산은 종가만 보므로 자금 경로에는 닿지 않는다.
    → 인트라바 옵션 자체는 2026-07-25 사용자 결정으로 손대지 않고 그대로 둔다.
