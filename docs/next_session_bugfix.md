...
#### C-2. BUG-20 — 새 리포트가 승계하지 않은 기존 미수정 건

**새 리포트는 07-27 리포트의 잔여 건을 승계하지 않았다.** "확정 20건"이 전부가 아니다.
07-27 잔여 중 최소 아래는 **여전히 열려 있다**(이번에 소스로 확인):
...
- 그 외 07-27 잔여(BUG-13 견적 0.01 초과 · BUG-23 SSE 유실 등)는
  `git show <이 커밋 이전>:docs/reports/bug_latest.md` 또는
  `docs/reports/bug_20260727_134231.md` 에서 확인할 것.
