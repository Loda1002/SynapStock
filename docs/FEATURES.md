...
| **멀티 종목(동시 매수)** | 하나의 예산·하나의 402 Guard 아래 N종목을 독립 포지션으로 동시 매수(1회=총 spend/N). 대시보드 종목 선택·포커스·종목별 요약, 백테스트 포트폴리오. 드라이 전용(라이브·추세·목시세 멀티 거부), N=1 하위호환 | `web/engine.py`·`scripts/backtest.py --symbols` | ✅ test_multistock 31건·웹 3종목 검증·적대검증 6건 수정 |
| **백테스트 러너** | 규칙 vs Gemini(엄격/추세) vs 추세추종 비교 + **매수후보유 벤치마크**·시장노출 (`--strategy trend --suffix _bear`) · `--symbols` 멀티 종목 포트폴리오(등가중 벤치마크) | `scripts/backtest.py` | ✅ 3종목 실측 |
