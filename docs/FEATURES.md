...
> - 계획이 아니라 **현황**이다 — 지금 실제로 무엇이 되어 있는가.
> - 큰 기능이 완료/변경되면 이 문서의 해당 행을 갱신한다.
>
> 마지막 갱신: 2026-08-01 · 대상 커밋: `c6ca807` · 코드 규모 **19,264줄**(agents 1,285 /
> payments 1,874 / market 688 / web 3,429 / shared 184 / scripts 11,107 / 루트 697 — 추적 중인
> `*.py` 실측. 앱 코드만 세면 8,157줄이고 나머지는 테스트·도구다)
...
멀티에이전트 시스템" (= SynapStock)

**현재(제출용)**: 제품명 **SynapStock**, 그 안의 지출 승인 게이트가 **402 Guard** 다.
*x402 는 구매자의 **키**를 지킨다. **무엇에 서명하는지**는 지키지 않는다. 402 Guard 는 사람 없이
결제하는 AI 에이전트를 위해, 돈이 나가기 직전에 청구서가 합의한 그 청구서인지 대조하는 마지막
검증 게이트다.* 토큰화 주식 자동매매는 이 게이트를 증명하는 레퍼런스 시나리오.

> ⚠ **옛 문안 *"x402 는 파는 쪽을 보호한다"* 는 쓰지 않는다** — 스펙에 `maxAmountRequired` 와
> 구매자 로컬 서명이 이미 있어 한 줄로 반박된다(2026-07-27 스펙 재확인).

**왜 바꿨나**: "AI가 알아서 매매한다"를 앞세우면 흔한 단일 전략 봇으로 분류돼 차별점이 묻힌다.
검증·가드레일 레이어를 정면에 세우면 킥오프가 지목한 자리에 앉는다.
근거·전환 상세는 [`docs/differentiation.md`](differentiation.md).

> ⚠ **"직전 x402 해커톤 수상작 16건 중 트레이딩 봇 0건" 은 인용하지 않는다** — 2026-07-27
> 조사에서 **출처 미확인 + 반례 3건**(Peaks·Clawpump·Agent Arc)으로 반증됐다. 재포지셔닝 결론
> 자체는 유지되지만 근거는 위 문장으로 대체한다. **매매 기능을 숨길 필요는 없다.**
...
| **증빙 아카이브** | tx 해시·전후 잔액·교차검증을 JSON으로 저장 | `artifacts/tx/` | ✅ **16파일 — localnet 10 · devnet 4 · 배포 세션 RPC 사후 검증본 1 · 거부 데모 로그 1**(비JSON. 검증본에도 `network` 키를 넣어 **집계상 devnet 은 5**로 잡힌다). **데모 영상 G 구간이 화면에 띄우는 증빙이 `20260803_102820_solana-devnet_web_session.json`** 이다(웹 대시보드 **라이브** 세션 · Circle 공식 민트 2.00 USDC · `cross_check` usdc_ok·stock_ok true · 결제 tx 의 Memo 가 x402 주문번호를 온체인에 결박). 같은 형식의 리허설본 `20260803_001823_*` 은 대본이 폴백으로 지목한다. devnet 대표 증빙은 `20260731_1508_solana-devnet_live_buy.json`(Circle 공식 민트) — ⚠ 옛 `20260724_1643_*` 은 자체발행 민트라 **인용 금지**. localnet 10건 중 2건은 매매 세션이 아니라 **온체인 예산 레일** 증빙(`*_delegation.json`, `wired_into_product:false`). 별도로 HTTP 402 실 TCP 왕복 로그 1건 `artifacts/x402_http/` |
...
| **07-24** | **종합 검토** | 자평 | 조사 7축·감사 64건·적대 검증 32건 |
...
> 상세·공수·순서는 [`docs/differentiation.md`](differentiation.md) §2.
...
| 축 | 공식 문구 | 우리 증거 | 강도 |
|---|---|---|---|
| ① 혁신성·UX·상업성 | 직관적·새로운 UX, 기존 문제 해결 | 402 Guard 재포지셔닝, 첫 화면 KPI(수익률 아님), **공격 재현 CLI**(`scripts/red_team.py --attacker <pubkey>` — 웹 화면에 공격 입력 UI 는 없다). 2026-07-28 보강: 소개서에 **경쟁 지도 1장**(AgentFabric·Circle Agent Wallets·Kyvern, "우리가 확인한 범위에서" 한정) + 수익모델 3단 단가(전부 '가정' 명시) | **중** — 2026-08-01 부서 재판정. 사유가 코드가 아니라 **제출물이 아직 실물로 없다**는 것으로 바뀌었는데, 그중 **①(소개서 PDF)은 2026-08-02 에 완성됐다** — `docs/artifacts/SynapStock_pitch_deck.pdf`(14p · A4 가로), 09장에 실제 대시보드 캡처(AI 두 레이어 실측치 포함)와 `red_team` 실제 출력을 넣었다. **남은 것은 ③(영상 미촬영) 하나다** |
