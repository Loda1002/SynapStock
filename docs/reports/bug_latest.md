...
| **BUG-24** (낮음) 숫자형 환경변수가 빈 값이면 임포트 단계에서 앱 전체가 죽음 | ✅ 수정 | `5f4c1a6` — `env_num` 신설(빈 값은 기본값 복원 · 잘못된 값은 **변수명을 밝히며** 중단). ⚠ 아래 수정안의 경고대로 `MAX_BUDGET_USDC` 만 `blank_ok=False` 로 두어 서버측 상한이 조용히 10배로 풀리지 않게 했습니다. 회귀 5묶음은 `test_dry_sell.py`(같은 계열 = 배포 환경변수에서만 터지는 결함)에 넣어 **테스트 파일 수는 23개 그대로**입니다 |

따라서 남은 것은 '낮음' **11건**·'정보' 1건입니다. ⚠ 심각도는 **재현된 금액 영향** 기준이라
