...
(() => {
  ...
  function notify(evt, title, body, cls) {
    ...
    if (document.hidden) {
      ...
      if (notifyEnabled()) {
        const n = new Notification("SynapStock — " + title, { body });
  ...
  /* ⚠ 라벨의 단일 출처는 여기다 — index.html 의 버튼 글자를 고쳐도 renderFeedToggle 이
     토글할 때마다 btn.textContent 로 다시 쓰기 때문에 첫 클릭에 되돌아간다.
     (▾ 도 같은 이유로 버튼 안 자식이 아니라 skeleton.css 의 ::after 로 그린다.) */
  const MORE_LABEL = {
    decisions: ["AI 판단 타임라인 보기", "AI 판단 타임라인 접기"],
    log: ["협상·이벤트 로그 보기", "협상·이벤트 로그 접기"],
  };
  ...
  /* 서랍으로 옮기는 카드들.
     · session-adv = 세션 고급 설정(실행 모드·재생 속도·시세 피드·데이터셋·봉 간격·AI 판단
                     모드·TA·DCA). 한 번 정해 두면 매번 건드릴 일이 없는 검증용 노브다.
     · history     = '지난 세션' 목록. 세션ID·Gemini 비율은 우리 계측 단위라 관람자가 읽을
                     것이 아니다(축④·축② 증거로서의 값은 영상·소개서가 대신 진다).
     ⚠ session(세션 설정)은 **여기 없다 — v13 에서 그리드로 되돌렸다**(사용자 지시 2026-08-01).
       그리드에 남는 것은 사람이 매번 고르는 것뿐이다: 전략 · 신호 · 살 종목 · 시작/종료.
       시안도 세션 카드를 첫 줄 오른쪽에 두고 design.css 가 span 4 · min-height 379 를 그
       자리에 맞춰 두었다. ⚠ 라이브 옵션 잠금(data-lab-only)은 그대로다 — ?lab=1 로만 풀린다.
     '한도 바꾸기'(AP2 재서명)도 여기 없다 — v11 에서 그리드로 되돌렸다. 예산 한도를 정하는 것은
     개발용 조작이 아니라 이 제품을 쓰는 사람의 일이고, 그 한도를 코드가 집행하는 것이 제품이다. */
  if (el.devSlot) {
    for (const sel of ['[data-card="session-adv"]', '[data-card="history"]']) {
      const card = $(sel);
